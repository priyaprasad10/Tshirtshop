...
from store.models import Tshirt,Brand,Color,IdealFor,NeckType,Sleeve,Occasion,SizeVariant,Cart,Payment,Order,OrderItem
...
class  TshirtConfiguration(admin.ModelAdmin):
    ...
    def get_image(self,obj):
        return format_html(f"""<a target="_blank" href='{obj.image.url}'><img height="50px" src='{obj.image.url}'/></a>""")

class CartConfiguration(admin.ModelAdmin):
    model=Cart
    #fields=('sizVariant','quantity','user')
...
class  OrderConfiguration(admin.ModelAdmin):
    ...
    readonly_fields=['user','shipping_address','phone','total','payment_method','payment','payment_request_id','payment_id','payment_status']

    fieldsets=(("Order Information",{'fields':('order_status','shipping_address','phone','total','user')}),("Payment Information",{
    ...
    def payment_id(self,obj):
        ...
        if (payment_id is None or payment_id==""):
            return "Payment Id is Not Avaiable"
        else:
            return payment_id    
